  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm px-4 pb-8">
      <div className="w-full max-w-sm bg-surface border border-border rounded-3xl p-6 animate-[slideUp_0.3s_ease-out]">
        ...
        {success && <p className="mt-3 text-accent text-sm">{success}</p>}
        ...
        {noResults && !inviteLink && (
          <div className="mt-4 text-center">
            <p className="text-text-secondary text-sm mb-3">No user found with that email or username</p>
            <button
            ...
            >
              {sending ? 'Generating…' : 'Invite via Link'}
            </button>
          </div>
        )}
        ...
        {inviteLink && (
          <div className="mt-4">
            <p className="text-accent text-sm font-semibold mb-2">
              {copied ? 'Invite link copied!' : 'Share this link with your friend:'}
            </p>
